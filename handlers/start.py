...
WELCOME_TEXT = (
    ...
    "Отправь мне учебный материал — текст, документ или фото — "
    "и я подготовлю интерактивные материалы для быстрого усвоения "
    "и пакет для NotebookLM. Ещё я умею генерировать промпты для "
    "презентаций и инфографики.\n\n"
    "Поддерживаемые форматы: изображения, PDF, DOCX, TXT и текст.\n"
    "Подробнее — /help"
)
...
HELP_TEXT = (
    ...
    "Для изображений дополнительно доступна кнопка "
    "«🧾 Исходный текст / OCR», кнопка «📋 Скопировать OCR» "
    "и кнопка «📄 Исходник .txt».\n\n"
    ...
    "Если изображений много, можно отправлять их несколькими сообщениями подряд: "
    "бот будет накапливать их в очередь, а потом обработает всё вместе по кнопке "
    "«Готово». Кнопка «Очистить» сбрасывает текущую очередь.\n\n"
    ...
    "Текстовые задачи — Groq (Llama 3.3 70B), изображения — GitHub Models (GPT-4o).\n\n"
    "*Команды:*\n"
    "/start — начало работы\n"
    "/help — эта справка"
)
